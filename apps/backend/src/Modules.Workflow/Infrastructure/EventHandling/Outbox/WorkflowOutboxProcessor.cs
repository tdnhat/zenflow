...
namespace Modules.Workflow.Infrastructure.EventHandling.Outbox
{
    public class WorkflowOutboxProcessor : BackgroundService
    {
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly ILogger<WorkflowOutboxProcessor> _logger;
        private readonly TimeSpan _interval = TimeSpan.FromSeconds(10);
        private readonly int _batchSize = 20;

        public WorkflowOutboxProcessor(
            IServiceScopeFactory scopeFactory,
            ILogger<WorkflowOutboxProcessor> logger)
        {
        ...
        {
            _logger.LogInformation("Workflow Outbox Processor started");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogError(ex, "Error processing workflow outbox messages");
                }
        ...
        private async Task ProcessOutboxMessagesAsync(CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Processing {Count} workflow outbox messages", messages.Count);
            ...
            foreach (var message in messages)
            {
                try
                {
                    ...
                    _logger.LogInformation("Published workflow event {EventType} with ID {Id}",
                        message.EventType, message.Id);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Error processing workflow outbox message {Id}", message.Id);
